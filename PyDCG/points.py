...
def _submit_point_set_list(P, species, comment=" ", user_id=None):
    ...
    for pts in P:
        ...
        # to avoid collisions
        date_discovered = datetime.datetime.today()
        idx = ''.join(random.choice(string.ascii_letters + string.digits)
                      for _ in range(10))
        name = str(len(pts))+"_"+species+"_" + \
            str(date_discovered.date())+"_"+idx+".sp"
        name = "temp_subs/"+name
        print(name)
        file_sp = open(name, "w")
...
def submit_rectilinear_crossing_number(pts, user_id=None, comment=None):
    _submit_point_set(pts, "rectilinear_crossing_number",
                      comment=comment, user_id=user_id)


def submit_empty_convex_pentagons(pts, user_id=None, comment=None):
    _submit_point_set(pts, "empty_convex_pentagons",
                      comment=comment, user_id=user_id)
...
def submit_empty_convex_quadrilaterals(pts, user_id=None, comment=None):
    _submit_point_set(pts, "empty_convex_quadrilaterals",
                      comment=comment, user_id=user_id)


def submit_empty_convex_hexagons(pts, user_id=None, comment=None):
    _submit_point_set(pts, "empty_convex_hexagons",
                      comment=comment, user_id=user_id)
